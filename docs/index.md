---
layout: default
title: Solar Proa - Wind-solar-powered Outrigger Boats
---

<div style="text-align: center; padding: 2em 0;">
  <h1 style="font-size: 2.5em; margin-bottom: 0.2em;">Prahu Surya Layar</h1>
  <p style="font-size: 1.3em; color: #666; margin-top: 0;">Wind-solar-powered Hybrid Vessels for the Tropics</p>
</div>

---

## Executive Summary

Project _Solar Proa_ is developing wind-solar-powered hybrid vessels for carbon-neutral small-scale tropical maritime transport, exploration, and tourism.
Completion of our 9-meter minimal viable product _Roti Proa II_ is targeted for mid-2026 and strategic partnerships are available for commercial validation.

---

## The Vision

Electric vehicle adoption across Asia has created new expectations for quiet, clean transportation - expectations that extend to recreational maritime experiences. We're bringing carbon-neutral propulsion to island-hopping, snorkeling, and diving tourism through vessels that combine:

- **Traditional Design** - Inspired by Austronesian outrigger proas and Indonesian sailing traditions
- **Modern Engineering** - Parametric CAD design, aluminum construction, efficient electric propulsion
- **Renewable Energy** - 4kW solar arrays plus sail propulsion for zero-emission operation

---

## Our Vessels

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 2em; margin: 2em 0;">
  <div style="border: 1px solid #ddd; padding: 1.5em; border-radius: 8px;">
    <h3>Roti Proa I</h3>
    <p><strong>Status:</strong> Completed & Tested (2025)</p>
    <p><strong>Size:</strong> 4.2m prototype</p>
    <p><strong>Achievement:</strong> Proof of concept validated in Singapore waters</p>
    <p><a href="{{ '/rp1.html' | relative_url }}">View Details & Renders →</a></p>
  </div>
  
  <div style="border: 1px solid #ddd; padding: 1.5em; border-radius: 8px; background: #f8f8f8;">
    <h3>Roti Proa II</h3>
    <p><strong>Status:</strong> Under Construction</p>
    <p><strong>Size:</strong> 9m day tourism vessel</p>
    <p><strong>Capacity:</strong> 4 passengers + 2 crew</p>
    <p><strong>Target:</strong> June 2026 completion</p>
    <p><a href="{{ '/rp2.html' | relative_url }}">View Details & Renders →</a></p>
  </div>
  
  <div style="border: 1px solid #ddd; padding: 1.5em; border-radius: 8px;">
    <h3>Roti Proa III</h3>
    <p><strong>Status:</strong> Design Phase</p>
    <p><strong>Size:</strong> 13m multi-day vessel</p>
    <p><strong>Capacity:</strong> 5-6 passengers</p>
    <p><strong>Target:</strong> 2027 validation</p>
    <p><a href="{{ '/rp3.html' | relative_url }}">View Details & Renders →</a></p>
  </div>
</div>

---

## Why Tropical Southeast Asia?

**Exceptional solar irradiation** makes the region ideally suited for sustainable maritime propulsion. Our designs optimize traditional outrigger configurations to maximize solar panel area while maintaining excellent sailing characteristics.

The shunting, single-outrigger layout provides:
- Ample space for solar arrays between hull and outrigger
- Crew positioning that avoids casting shadows on panels
- Superior stability and efficiency for electric propulsion

---

## Commercial Opportunity

**Eco-Tourism Applications:**
- Island-hopping and coastal exploration
- Snorkeling and diving expeditions  
- Educational sailing experiences
- Wildlife observation cruises

**Operational Advantages:**
- Zero fuel costs and carbon emissions
- Significantly lower maintenance than diesel
- Quiet operation enhancing wildlife experiences
- Regulatory advantages from green certifications

**Market Position:**
At 10 knots under motor power, Roti Proa II targets the contemplative pace of eco-tourism - where the journey itself is part of the value proposition.

---

## Technical Innovation

All vessel designs are created using **parametric CAD modeling in FreeCAD and Python**, enabling:

- Rapid design iteration and optimization
- Automated generation of construction documentation
- Version control and design transparency
- Open-source accessibility for educational purposes

**[Explore the CAD Models →]({{ '/technical.html' | relative_url }})**

---

## Current Status & Timeline

**Q1 2026:** Main hull delivery (9m Roti Proa II)  
**Q2 2026:** Final assembly and sea trials  
**Q3 2026:** Commercial validation with strategic partners  
**2027+:** Pilot production and 13m vessel development

---

## Partnership Opportunities

We welcome strategic partners for:

- **Field testing collaboration** and operational feedback
- **Joint development** of eco-tourism protocols
- **Co-branding opportunities** for sustainability leadership
- **CSR alignment** through maritime innovation support

Partnership structures can be flexibly tailored to organizational priorities.

---

## Project Team

**Project Coordinator:** A/Prof Martin Henz, School of Computing, National University of Singapore

**Technical Advisors:** Naval architect, naval engineer, electrical engineer

**Support:** National University of Singapore, industry sponsors, private donors

---

## Contact

**Email:** henz@nus.edu.sg  
**Institution:** School of Computing, National University of Singapore

---

<div style="text-align: center; padding: 2em 0; color: #666; font-size: 0.9em;">
  <p><em>Combining Southeast Asian maritime traditions with modern sustainable technology</em></p>
  <p>This site and all CAD models are automatically built and deployed via GitHub Actions</p>
</div>
