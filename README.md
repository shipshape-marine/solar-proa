...
All design components are specified in software, in `src` as the
_single source of truth_. Features include:
- Main macro: The boat designs are generated by the _Solar Proa Design Generator_,  [`src/SolarProa.FCMacro`](https://github.com/solar-proa/CAD/blob/main/src/SolarProa.FCMacro) from [FreeCAD](https://www.freecad.org/).
- Parameters: The boat is fully parameterized; the files [`constants/boats/RP*.json`](https://github.com/solar-proa/CAD/blob/main/constants/boats/) let you choose the numbers of solar panels and the size of all components.
...
- Rotation: Some parts such as the rigs and rudder can be rotated or moved (see file [`rotation.py`](https://github.com/solar-proa/CAD/blob/main/src/design/rotation.py)) in the configuration files.
