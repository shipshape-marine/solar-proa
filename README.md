...
Design-specific components live in `src/` while boat-independent analysis is provided by the [shipshape](https://github.com/solar-proa/shipshape) library. Features include:
- Main macro: The boat designs are generated by the _Solar Proa Design Generator_,  [`SolarProa.FCMacro`](https://github.com/solar-proa/CAD/blob/main/SolarProa.FCMacro) using [FreeCAD](https://www.freecad.org/).
- Parameters: The boat is fully parameterized; the files [`constants/boats/rp*.json`](https://github.com/solar-proa/CAD/blob/main/constants/boats/) let you choose the numbers of solar panels and the size of all components. SolarProa-specific derived parameters are computed by [`src/parameter/compute.py`](https://github.com/solar-proa/CAD/blob/main/src/parameter/compute.py), loaded as a plugin by `shipshape.parameter`.
- Configurations: Moving parts such as rig, sails, rudders etc can be configured in the files [`constants/configurations/*.py`](https://github.com/solar-proa/CAD/blob/main/constants/configurations), for example [`closehaul.json`](https://github.com/solar-proa/CAD/blob/main/constants/configurations/closehaul.json).
- Mirroring: Solar proas have a transversal symmetry axis. Most components are described in file [`mirror.py`](https://github.com/solar-proa/CAD/blob/main/src/design/mirror.py) and automatically mirrored in the generated model.
- Rotation: Some parts such as the rigs and rudder can be rotated or moved independently (see file [`rotation.py`](https://github.com/solar-proa/CAD/blob/main/src/design/rotation.py)) in the configuration files.
